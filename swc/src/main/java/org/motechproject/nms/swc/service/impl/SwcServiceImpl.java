...
//            (record.getLanguage() != null)
    ...
    @Override
    public Swachchagrahi getByMctsSwcIdAndPanchayat(final String mctsSwcId, final Panchayat panchayat) {
        if (mctsSwcId == null || panchayat == null) {
            LOGGER.error(String.format("Attempt to look up SWC by a null mctsSwcId (%s) or panchayat (%s)",
                    mctsSwcId, panchayat == null ? "null" : panchayat.getName()));
            return null;
        }
        ...
        QueryExecution<Swachchagrahi> queryExecution = new QueryExecution<Swachchagrahi>() {
            ...
            public Swachchagrahi execute(Query query, InstanceSecurityRestriction restriction) {
                query.setFilter("swcId == _mctsSwcId && panchayat == _panchayat");
                query.declareParameters("String _mctsSwcId, org.motechproject.nms.region.domain.Panchayat _panchayat");
                query.setClass(Swachchagrahi.class);
                query.setUnique(true);

                return (Swachchagrahi) query.execute(mctsSwcId, panchayat);
            }
        };
